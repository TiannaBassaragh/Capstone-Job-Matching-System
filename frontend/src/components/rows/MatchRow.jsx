...
function ExpandedContent({ match, skills, onViewDetails }) {
    const hasSkills = skills.strong.length > 0 || skills.partial.length > 0
        || skills.low.length > 0 || skills.unclear.length > 0 || skills.missing.length > 0;
    return (
        <div className="expanded">
            <div className="expanded-label">Competency match breakdown</div>
            <div className="expanded-skills">
                {hasSkills ? (
                    <>
                        ...
                        {skills.low.map(s     => <span key={s} className="skill skill--low">{s}</span>)}
                        {skills.unclear.map(s => <span key={s} className="skill skill--unclear">{s} ?</span>)}
                        {skills.missing.map(s => <span key={s} className="skill skill--missing">{s}</span>)}
                    </>
                ) : (
