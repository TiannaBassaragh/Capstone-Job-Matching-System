...
// {
//     type: "preset" | "custom",
//     key:  "all" | "active" | "expiring" | "inactive" | "custom",
//     label: string,
//     rules?: {
...
export function filterJobs(
    ...
    if (filterConfig.type === "preset") {
        switch (filterConfig.key) {
            case "active":
                return jobs.filter((job) => job.status === "active" || job.status === "expiring");
    ...
    if (filterConfig.type === "custom") {
        const { matches } = filterConfig.rules;

        return jobs.filter((job) => {
            return matches.active ? job.matches >= matches.value : true;
        });
    }
